...
void LCD_init(void);
void display_mag(unsigned char);
void display_prox(float);
...
// A threshold value for the IR sensor output
float threshold = 3.0;
...
{
	unsigned char msg1[] = "MAG FIELD: ", msg2[] = "IR SENSOR: ", i;
	LCD_CMD(0x38);
	LCD_CMD(0x0E);
	LCD_CMD(0x01);
	LCD_CMD(0x80);
...
void display_mag(unsigned char magnet) // Subroutine to display Hall Effect Sensor data on LCD
{
	LED_1 = magnet;
	LCD_CMD(0x8B);
	LCD_DATA(magnet+48);
...
void display_prox(float value) // Subroutine to display IR Sensor data on LCD
{
	unsigned char i;
	LCD_CMD(0xCB);
